...
def get_voice_id() -> str:
    """
    Get ElevenLabs voice ID from environment.

    Returns:
        Voice ID string

    Exits:
        If ELEVENLABS_VOICE_ID not found
    """
    _load_env()
    voice_id = os.getenv("ELEVENLABS_VOICE_ID")
    if not voice_id:
        project_root = Path(__file__).resolve().parent.parent.parent
        print("ERROR: ELEVENLABS_VOICE_ID not found in environment", file=sys.stderr)
        print(f"Add ELEVENLABS_VOICE_ID to .env file at: {project_root}/.env", file=sys.stderr)
        sys.exit(1)
    return voice_id
